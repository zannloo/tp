...
public class EditCommandParser implements Parser<EditCommand> {
    ...
    public EditCommand parse(String args) throws ParseException {
        ...
        if (argMultimap.getValue(OPTION_NAME).isPresent()) {
            editPersonDescriptor.setName(ParserUtil.parseName(argMultimap.getValue(OPTION_NAME).get()));
        }
        if (argMultimap.getValue(OPTION_PHONE).isPresent()) {
            editPersonDescriptor.setPhone(ParserUtil.parsePhone(argMultimap.getValue(OPTION_PHONE).get()));
        }
        if (argMultimap.getValue(OPTION_EMAIL).isPresent()) {
            editPersonDescriptor.setEmail(ParserUtil.parseEmail(argMultimap.getValue(OPTION_EMAIL).get()));
        }
        if (argMultimap.getValue(OPTION_ADDRESS).isPresent()) {
            editPersonDescriptor.setAddress(ParserUtil.parseAddress(argMultimap.getValue(OPTION_ADDRESS).get()));
        }
        parseTagsForEdit(argMultimap.getAllValues(OPTION_TAG)).ifPresent(editPersonDescriptor::setTags);
